...
fn main() {
    match run() {
        Ok(()) => println!("tutorial thrift_tutorial_server ran successfully"),
        Err(e) => {
            println!("tutorial thrift_tutorial_server failed with error {:?}", e);
            std::process::exit(1);
        }
    }
}
...
fn run() -> thrift::Result<()> {
    let options = clap_app!(rust_tutorial_server =>
        ...
        (about: "Thrift Rust tutorial thrift_tutorial_server")
        (@arg port: --port +takes_value "port on which the tutorial thrift_tutorial_server listens")
    );
    ...
    let i_prot_fact = TBinaryInputProtocolFactory::new(); //TCompactInputProtocolFactory::new();
    ...
    // create the thrift_tutorial_server and start listening
